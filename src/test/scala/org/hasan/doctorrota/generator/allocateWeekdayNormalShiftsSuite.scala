...
class allocateWeekdayNormalShiftsSuite extends AnyFunSuite with Matchers {
  test("can allocate normal weekday shifts") {
    // Given:
    ...
    // When:
    allocateWeekdayNormalShifts(Seq(weeklyRota), doctors)
