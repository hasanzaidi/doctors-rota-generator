...
class ShiftSuite extends AnyFunSuite with Matchers {
  test("should generate different shifts") {
    val longDayShift = ShiftFactory(ShiftType.LONG_DAY, LocalDate.of(2020, 1, 31))
    ...
    longDayShift.shiftType should equal(ShiftType.LONG_DAY)

    val nightShift = ShiftFactory(ShiftType.NIGHT, LocalDate.of(2020, 1, 31))
    ...
    nightShift.shiftType should equal(ShiftType.NIGHT)

    val normalShift = ShiftFactory(ShiftType.NORMAL, LocalDate.of(2020, 1, 31))
    ...
    normalShift.shiftType should equal(ShiftType.NORMAL)
  }
}
